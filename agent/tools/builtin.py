...
def run_bash(command: str, timeout: int = 60, limit: int = 4000) -> str:
    ...
    Команда выполняется без терминала: стандартный ввод не подключён, за клавиатурой никого нет.
    Скрипт, вызывающий input(), сразу упадёт с EOFError — это не поломка окружения. Если нужны
    данные от человека, спрашивай через ask_user. Если пользователь просил именно интерактивный
    скрипт, написать его можно, но запускать для проверки нужно с подачей ввода:
    printf '5\\n' | python script.py. Если программой будет пользоваться человек, запускай её
    через run_in_terminal.

    На Windows это Git Bash, и пути вида /c/sber понимает только сам bash. Программы Windows —
    python, pip и прочие — такой путь не откроют и вернут FileNotFoundError: им нужен обычный
    путь, C:/sber/file.py. Сомневаешься в текущем каталоге — узнай его командой pwd -W.
...
def run_powershell(command: str, timeout: int = 60, limit: int = 4000) -> str:
    ...
    Команда выполняется без терминала: стандартный ввод не подключён. Скрипт, вызывающий input()
    или Read-Host, сразу упадёт. Спрашивать человека нужно через ask_user, а интерактивный скрипт
    для проверки запускать с подачей ввода.
