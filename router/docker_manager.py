...
class DockerManager:
    ...
    def restart_client(self, login: str, db_port: int, rabbitmq_port: int,
                                    rabbitmq_mgmt_port: int) -> dict:
        logger.info(f"Restarting EXISTING infrastructure for {login} with ports: DB={db_port}, RMQ={rabbitmq_port}")
    ...
    def _run_compose(self, login: str, db_port: int, rabbitmq_port: int, rabbitmq_mgmt_port: int, build: bool):
        env = os.environ.copy()
        env.update({
            "LOGIN": login,
            "DB_PORT": str(db_port),
