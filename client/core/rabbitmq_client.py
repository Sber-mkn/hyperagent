        ...
        try:
            ...
            if message_type == "ready":
                ...
                if self._can_run_command(command_info):
                    ...
                    rpc_response = {"result": result_text}
                    display_result = {
                        "returncode": 1 if result_text.startswith("[ошибка инструмента") else 0,
                        "stdout": result_text,
                        "stderr": "",
                    }
                else:
                    rpc_response = {"result": "[denied by user: read-only access]"}
                    display_result = {
                        "returncode": 1,
                        "stdout": "",
                        "stderr": "Denied by user (read-only access)",
                    }
                self._emit(
...
def _describe_command(command: dict) -> str:
    if not isinstance(command, dict):
        return str(command)
    name = command.get("name") or "unknown_tool"
    arguments = command.get("arguments")
    return f"{name}({arguments})" if arguments is not None else f"{name}()"
